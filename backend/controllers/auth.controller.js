...
const authCtrl = {} 
    ...
    if(!req.headers.authorization){ res.json({ 'status':'0', 'msg': 'Unauthorized request.'}) } 
    ...
    if(token==null){ 
        res.json({ 'status':'0', 'msg': 'Requerimiento no autorizado'}); 
    }else{ 
        try 
        { const payload = jwt.verify(token, "secretkey"); 
        //payload retorna la información del user que se uso en el método de login 
        req.userId = payload._id; next();
        //se pasa a procesar el siguiente método del stack de la peticion 
    } catch (error) { 
